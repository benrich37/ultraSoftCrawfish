...
def get_pdos_weights_sabcj(idcs, path, data, orb_bool_func):
    ...
    if not orb_bool_func is None:
        el_orb_u_dict = get_el_orb_u_dict(path, atoms, data.orbs_dict, idcs)
        ...
        for idx in idcs:
            orbs += data.orbs_dict[kmap[idx]]
    weights_sabcj = np.zeros(np.shape(data.E_sabcj))
    for orb in orbs:
        weights_sabcj += np.abs(data.proj_sabcju[:,:,:,:,:,orb])**2
...
def get_pdos_pieces(idcs, path, data, res, orbs, Erange):
    ...
    return Erange, weights_sabcj, data.E_sabcj
